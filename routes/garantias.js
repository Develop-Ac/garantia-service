...
/**
 * Constrói a citação do histórico (HTML e texto) no formato padrão:
 * De / Enviada em / Para / Cc / Assunto + corpo.
 * Observações:
 *  - Para e Cc só são totalmente conhecidos nos e-mails enviados no momento do envio. Aqui,
 *    quando faltarem, usamos defaults (para o nosso endereço e/ou vazio).
 *  - Para e Cc de mensagens "Recebidas" normalmente não vêm no webhook; exibimos pelo menos "De",
 *    "Enviada em" e "Assunto".
 */
async function buildQuotedThread(client, garantiaId, defaults = {}) {
  const { ourAddress = (process.env.EMAIL_FROM || ''), toDefault = '', ccDefault = '' } = defaults;

  const q = `
    SELECT id, tipo_interacao, descricao, assunto, data_ocorrencia
    FROM historico_garantias
    WHERE garantia_id = $1
      AND tipo_interacao IN ('Email Enviado','Resposta Enviada','Resposta Recebida')
    ORDER BY data_ocorrencia DESC
    LIMIT ${THREAD_LIMIT};
  `;
  const { rows } = await client.query(q, [garantiaId]);

  const htmlParts = [];
  const textParts = [];

  // percorremos do mais recente para o mais antigo e empilhamos
  for (const it of rows) {
    const subject = it.assunto || '(sem assunto)';
    const sentAt = fmtDateBR(it.data_ocorrencia);
    let from = '';
    let to = '';
    let cc = '';
    let bodyHtml = '';
    ...
    if (/Recebida/i.test(it.tipo_interacao)) {
      // webhook salva: <b>De:</b> NAME <mail@x> <br> <hr> HTML
      const m = /<b>De:<\/b>\s*(.*?)<br>/i.exec(it.descricao || '');
      from = m ? m[1] : 'Desconhecido';
      to = ourAddress || '';
      cc = '';
      const parts = String(it.descricao || '').split(/<hr\s*\/?>/i);
      bodyHtml = parts.length > 1 ? parts.slice(1).join('<hr>') : (it.descricao || '');
      bodyText = bodyHtml.replace(/<[^>]+>/g, '').trim();
    } else {
      // Enviado/Resposta Enviada — armazenamos só o corpo; remetente somos nós
      const fromNameOrAddr = process.env.EMAIL_FROM_NAME
        ? `${process.env.EMAIL_FROM_NAME} <${ourAddress}>`
        : ourAddress || 'Gestão de Qualidade';
      from = fromNameOrAddr;
      to = toDefault;
      cc = ccDefault;
      bodyHtml = (it.descricao || '').replace(/\n/g, '<br>');
      bodyText = (it.descricao || '').trim();
    }
...
router.get('/garantias', async (req, res) => {
  ...
  try {
    const queryText = `
      ...
      FROM garantias g
      LEFT JOIN (
        SELECT garantia_id, json_agg(historico_garantias.* ORDER BY data_ocorrencia DESC) as historico
        FROM historico_garantias GROUP BY garantia_id
      ) h ON g.id = h.garantia_id
      LEFT JOIN (
        SELECT garantia_id, json_agg(anexos_garantias.* ORDER BY data_upload ASC) as anexos
        FROM anexos_garantias GROUP BY garantia_id
...
// POST /api/garantias (salva assunto e inclui CC)
...
router.put('/garantias/:id/status', async (req, res) => {
  ...
  try {
    ...
    const values = [novo_status, ...fields.map(f => extraData[f]), id];

    let updateQuery = `UPDATE garantias SET status = $1`;
    if (setClauses) updateQuery += `, ${setClauses}`;
...
// POST /api/garantias/:id/update (envio com citação completa + CC vindo da tela)
router.post('/garantias/:id/update', upload.array('anexos', 10), async (req, res) => {
  ...
  try {
    ...
    if (enviar_email === 'true') {
      ...
      const copiasEmailBD = garantiaInfo.copias_email;
      const destinatarioFinal = (destinatario || emailPrincipal || '').trim();
      if (!destinatarioFinal) throw new Error('Destinatário não encontrado.');

      // CC (prioridade: body > BD)
      const copiasOverride = (req.body.copias || req.body.cc || '').trim();
      const ccFinal = copiasOverride || copiasEmailBD || null;

      // cadeia de message-ids
      const refsResult = await client.query(
        `SELECT message_id, tipo_interacao, assunto
           FROM historico_garantias
      ...
      const chain = refsResult.rows
        .map(r => ({ id: normalizeMsgId(r.message_id), tipo: r.tipo_interacao, assunto: r.assunto }))
        .filter(r => !!r.id);
      ...
      // assunto (usa o último assunto conhecido, preservando "Re:")
      const lastSubjectRow = await client.query(
        `SELECT assunto FROM historico_garantias
          WHERE garantia_id = $1 AND assunto IS NOT NULL
          ORDER BY data_ocorrencia DESC LIMIT 1`,
        [id]
      );
      const lastSubject = lastSubjectRow.rows[0]?.assunto || null;
      ...
      // citação completa do histórico
      const { html: quotedHtml, text: quotedText } = await buildQuotedThread(
        client,
        id,
        { ourAddress: process.env.EMAIL_FROM, toDefault: destinatarioFinal, ccDefault: ccFinal || '' }
      );

      // corpo do e-mail (sua nova resposta + histórico citado)
      const bodyHtml = `<p>${String(descricao || '').replace(/\n/g, '<br>')}</p>
                        <br>
                        <p>Atenciosamente,<br>Equipa de Qualidade<br>AC Acessórios.</p>
                        ${quotedHtml}`;
...
${quotedText}`;
      ...
      if (parentId) {
        ...
        emailData.references = referencesArray;
        emailData.headers = {
...
// GET /dados-erp/venda/:ni (inalterado)
router.get('/dados-erp/venda/:ni', async (req, res) => {
  ...
  try {
    ...
    const response = {
      cliente: { CLI_CODIGO: cliCodigo, CLI_NOME: cliente.CLI_NOME, EMAILS: emails },
...
router.post('/garantias/email-reply', async (req, res) => {
  ...
  try {
    ...
    const duplicateResult = await client.query('SELECT id FROM historico_garantias WHERE message_id = $1', [message_id]);
    if (duplicateResult.rows.length > 0) {
      console.log(`Webhook N8N: E-mail duplicado ${message_id} ignorado.`);
      await client.query('COMMIT');
